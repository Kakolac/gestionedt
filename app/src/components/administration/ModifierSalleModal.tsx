...
export function ModifierSalleModal({ open, onClose, row }: Props) {
  const [state, formAction, pending] = useActionState(updateSalleAction, initial);
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="modifier-salle-titre"
      ...
      >
        ...
        <form action={formAction} className="mt-4 flex flex-col gap-4">
          <input type="hidden" name="salleId" value={row.id} />
          ...
          <fieldset className="flex flex-col gap-2 text-sm">
            ...
            <label className="flex cursor-pointer items-center gap-2">
              <input
                ...
                name="kind"
                value="classique"
                defaultChecked={row.kind === "classique"}
                disabled={pending}
            ...
            <label className="flex cursor-pointer items-center gap-2">
              <input
                ...
                name="kind"
                value="specifique"
                defaultChecked={row.kind === "specifique"}
                disabled={pending}
                className="text-indigo-600"
              />
