...
type Props = {
  ...
  identiteLabel: string | null;
};

const initial: ProfesseurActionState | undefined = undefined;

export function SupprimerProfesseurConfirmModal({
  open,
  onClose,
  professeurId,
  identiteLabel,
  ...
  if (!open || !professeurId || !identiteLabel) {
    return null;
  }
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="supprimer-professeur-titre"
        className="w-full max-w-[min(92vw,24rem)] rounded-2xl border border-white/60 bg-white p-6 shadow-xl"
        onMouseDown={(e) => e.stopPropagation()}
      >
        <h2
          id="supprimer-professeur-titre"
          className="text-lg font-semibold text-slate-900"
        >
          Supprimer ce professeur ?
        </h2>
        <p className="mt-2 text-sm text-slate-600">
          <strong>{identiteLabel}</strong> sera retiré du référentiel. Les
          affectations ou plannings futurs devront être ajustés séparément.
        </p>

        <form action={formAction} className="mt-6 flex flex-wrap gap-2">
          <input type="hidden" name="professeurId" value={professeurId} />
          {state && !state.ok ? (
