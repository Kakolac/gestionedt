/**
 * Initialise la visibilité matrice pour la tuile **Gestion des vacances** (`hub.gestion_vacances`)
 * sur les configs existantes : copie les slugs `MetierRole` de **`hub.creation_formation`**
 * lorsque `hub.gestion_vacances` est absent. Sinon ne modifie rien pour ce document (idempotent).
 *
 ...
 *
 * Usage : depuis **`app/`** — `npx tsx scripts/init/migrate-hub-gestion-vacances-tile.ts`
 */
...
const TARGET_TILE = "hub.gestion_vacances";
...
async function main(): Promise<void> {
  ...
  for (const d of docs) {
    ...
    if (Array.isArray(source) && source.every((x) => typeof x === "string")) {
      ...
      next[TARGET_TILE] = [];
      console.warn(
        `[migrate-hub-gestion-vacances-tile] Config ${String(d._id)} : pas de liste valide pour ${SOURCE_TILE}, ${TARGET_TILE} → [].`
      );
    }
  ...
  console.info(
    `Migration tuile gestion des vacances : ${updated} document(s) MenuVisibilityConfig mis à jour.`
  );
